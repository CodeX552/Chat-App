  ...
  <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-x1 overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
    <div className='p-5'>
      ...
      <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
        ...
        <input type="text" className='bg-transport border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search User...'/>
      </div>
    </div>
    <div className='flex flex-col'>
      {userDummyData.map((user, index)=>(
        <div onClick={()=>{setSelectedUser(user)}}
         key={index} className={`relative flex items-center gap-3 p-3 p1-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
